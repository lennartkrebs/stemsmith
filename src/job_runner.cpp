...
job_runner::job_runner(job_config base_config, model_cache& cache, std::filesystem::path output_root, std::size_t worker_count)
    : job_runner(std::move(base_config), make_engine(cache, std::move(output_root)), worker_count)
{}

job_runner::job_runner(job_config base_config, separation_engine engine, std::size_t worker_count)
...
void job_runner::process_job(const job_descriptor& job, const std::atomic_bool& stop_flag)
{
    ...
    const auto result = engine_.process(job);
...
void job_runner::handle_event(const job_event& event)
{
...

    {
        ...
        auto ctx_it = contexts_.find(input_path);
...
std::shared_ptr<job_runner::job_context>
job_runner::context_for(const std::filesystem::path& path) const
{
    std::lock_guard lock(mutex_);
    auto it = contexts_.find(path);
    if (it == contexts_.end())
    {
        return {};
    }
